...
async function create(templateName, projectName, options) {
  ...
  if (fs.existsSync(projectDir)) {
    ...
    if (isCurrentDir) {
      ...
      const { action } = await inquirer.prompt([{
        name: 'action',
        type: 'list',
        message: `Target directory ${chalk.cyan(projectDir)} already exists, please pick an action:`,
        choices: [
          {
            name: 'Overwrite',
            value: 'overwrite'
  ...
  // 目录不存在则开始创建项目
  process.env.ISSUES_BLOG_CLI_TEMPLATE_NAME = templateName;
...
module.exports = function issuesBlogCreate(templateName, projectName, ...args) {
  return create(templateName, projectName, ...args)
    .catch(err => {
      pauseSpinner();
      console.error(err);
      process.exit(1);
