...
const { readTemplateFromJson, writeTemplateToJson } = require('./utils/read-template-data')

const sshRegexp = /^git@github.com:(.+)\/(.+).git$/;
const httpsRegexp = /^https:\/\/github.com\/(.+)\/(.+).git$/;
const generateAddress = (username, repoName) => `github:${username}/${repoName}`;
...
function formatGitRepoAddress(gitRepoAddress) {
  ...
  throw new Error(`Format git repo address failed, cannot process this address ${gitRepoAddress}`);
}
...
async function addProjectTemplate(templateName, gitRepoAddress) {
  ...
  if (!isGitUrl(gitRepoAddress)) {
    console.log(`  ${chalk.red(`git repo address ${gitRepoAddress} is not a correct git address, please check the address.`)}`);
    return;
  }
...
module.exports = function addTemplate(templateName, gitRepoAddress) {
  return addProjectTemplate(templateName, gitRepoAddress)
    .catch(err => {
      console.error(err);
      process.exit(1);
    })
