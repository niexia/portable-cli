...
const { readTemplateFromJson, writeTemplateJson } = require("./utils/read-template-data");
...
async function deleteProjectTemplate(templateName) {
  ...
  if (!templateJson[templateName]) {
    console.log(`  ${chalk.red(`template name ${templateName} is not existing.`)}`);
    return;
  }
...
module.exports = function deleteTemplate(templateName) {
  return deleteProjectTemplate(templateName)
    .catch(err => {
      console.error(err);
      process.exit(1);
    })
